...
def parse_option():
    ...
    parser.add_argument('--load_ckpt', type=bool, default=False, choices=[True, False])
...
def set_loader(opt):
    ...
    #     transforms.ToTensor(),
    # ])
    train_dataset = AlzhDataset(type=opt.class_type, root=os.path.join(opt.dataset_path, 'train'), transform=transforms.Compose(train_transform))
    valid_dataset = AlzhDataset(type=opt.class_type, root=os.path.join(opt.dataset_path, 'validation'), transform=transforms.Compose(train_transform))
