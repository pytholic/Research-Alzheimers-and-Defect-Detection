...
i = 0
for image in glob.glob(opts.input + '/*.png'):
   with torch.no_grad():
    ...
    if opts.trainer == 'MUNIT':
        ...
        for j in range(opts.num_style):
            ...
            outputs = (outputs + 1) / 2.
            path = os.path.join(opts.output_folder, 'output{}{:03d}.jpg'.format(i,j))
            vutils.save_image(outputs.data, path, padding=0, normalize=True)
    elif opts.trainer == 'UNIT':
        outputs = decode(content)
        outputs = (outputs + 1) / 2.
        path = os.path.join(opts.output_folder, 'output{}.jpg'.format(i))
        vutils.save_image(outputs.data, path, padding=0, normalize=True)
    else:
        pass
   i += 1
